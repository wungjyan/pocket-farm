  ...
  <view class="pf-page production-select-page">
    <view class="pf-page-content">
      ...
      <view class="production-list">
        <view class="whole-plot-card pf-card" @click="chooseProduction(null)"><view><text class="production-name">整个地块</text><text class="production-meta">不关联具体种养批次</text></view><uv-icon name="checkmark" size="20" color="#2F7D4A" /></view>
        ...
        <view v-else v-for="item in productions" :key="item.id" class="production-card pf-card" @click="chooseProduction(item)"><view><text class="production-name">{{ productionName(item) }}</text><text class="production-meta">{{ productionMeta(item) }}</text></view><uv-icon name="arrow-right" size="17" color="#929A93" /></view>
      </view>
    </view>
  </view>
</template>
...
const plotId = ref(0); const productions = ref<Production[]>([]); const loading = ref(false); const loadError = ref(""); let openerEventChannel: OpenerEventChannel | null = null;
