...
Phase 8 已在后续阶段完成，详见下一节。

## Phase 8 交付记录

### 地块聚合详情

- 新增 `GET /api/v1/plots/{plotId}/detail`。接口先完成当前用户的 FarmMember 校验，再聚合返回地块基本信息、当前／历史种养、农事与收获记录及两类记录总数。
- HarvestRecord 仍经由 `Production → Plot` 聚合，不新增或回填 `plot_id`；FarmOperation 与 HarvestRecord 的原有编辑、锁定和归属规则均未改变。
- 聚合接口内的农事与收获记录按业务时间倒序返回，最多携带 100 条；总数独立返回，完整记录继续由既有分页列表接口提供。

### 小程序

- 地块详情改为使用聚合接口加载种养和生产记录数据，保留现有概览、快捷操作、当前／历史种养和独立记录页入口的页面结构。
- 生产记录入口改为展示农事与收获的真实总数，不新增说明性文案、嵌套卡片或多余视觉层级。

### 验证结果

- `uv run alembic current`：通过，当前为 `b8e4d2a1c673 (head)`。
- `uv run alembic check`：通过，无待生成 migration。
- `uv run pytest`：通过（36 passed）。
- `uv run ruff check .` 与 `uv run ruff format --check .`：通过。
- `pnpm --dir apps/miniapp exec vue-tsc --noEmit -p tsconfig.json`：通过。
- `pnpm --dir apps/miniapp run build:mp-weixin`：通过；仅有现有 Sass API 与 `@import` 弃用警告。
...
- 三个 Tab 与首批核心二级页已统一到同一套 Token、导航和页面层级；Phase 7 的结束种养页和 Phase 8 的地块聚合详情均沿用该基线，后续新页面继续沿用。
