...
## 更新摘要
**变更内容**
- CreatePlotRequest 架构已更新，现在要求所有三个字段（plot_type、area_value、area_unit）为必填而非可选
- 增强了验证约束以确保 API 级别的数据完整性，并提供缺失必填字段的适当错误处理
- 更新了地块创建接口的请求体规范和校验规则说明
...
**重要更新**：CreatePlotRequest 架构现已要求所有三个关键字段（plot_type、area_value、area_unit）为必填参数，不再支持可选字段，确保数据完整性和业务逻辑的一致性。
...
- **更新**：请求体字段（CreatePlotRequest）现在要求所有关键字段为必填：
  - name：必填，长度 1-100
  - plot_type：必填，地块类型枚举（FIELD/PADDY/GREENHOUSE/ORCHARD/FOREST/POND/BARN/OTHER）
  - area_value：必填，正数 Decimal
  - area_unit：必填，面积单位枚举（MU/SQUARE_METER/HECTARE）
  - boundary：可选，JSON 对象，必须包含 coordinateSystem 且值为 GCJ02
- 校验规则：
  - 所有三个面积相关字段（plot_type、area_value、area_unit）现在都是必填项
...
Validate --> |缺少必填字段| Err422["返回 VALIDATION_ERROR"]
...
- 请求体字段（UpdatePlotRequest）：
  ...
  - plot_type：可选，地块类型枚举
  - area_value：可选，正数 Decimal
  - area_unit：可选，面积单位枚举
  - boundary：可选，JSON 对象，coordinateSystem 必须为 GCJ02
- 校验规则：
  - 若提供 area_value 或 area_unit 中的任意一个，则必须同时提供另一个
  - boundary.coordinateSystem 必须为 GCJ02
...
- 换算规则：
  ...
  - 平方米保持不变
- 存储策略：
  - area_value 与 area_unit 为用户输入的真实值
  - area_m2 由二者换算得到，用于统一度量与展示
...
- 地块属性：
  - name：必填，长度限制
  - plot_type：创建时必填，固定枚举
  - area_value/area_unit：创建时必填，但需成对出现
  - area_m2：服务端计算得出
  - boundary：可选，坐标系统约束
...
- 422 校验失败：
  - **新增**：常见原因：缺少必填字段（plot_type、area_value、area_unit）
  - 常见原因：area_value/area_unit 未成对、boundary.coordinateSystem 不为 GCJ02、name 为空等
  - 定位参考：Pydantic 模型校验器
- 401 未授权：
  - 常见原因：缺少或无效令牌
...
**重要改进**：通过强制要求 CreatePlotRequest 中的所有关键字段，确保了数据完整性，减少了后续业务逻辑中的空值处理复杂度，提高了系统的健壮性。
...
- 创建：POST /api/v1/farms/{farm_id}/plots
  - **更新**：现在要求所有三个关键字段（plot_type、area_value、area_unit）为必填
...
### 校验与权限要点
- **更新**：创建地块时 plot_type、area_value、area_unit 三个字段现在都是必填项
- 面积单位与数值必须成对出现（编辑时）
- 边界坐标系统必须为 GCJ02
...
- [plot_service.py:90-104](file://apps/api/app/services/plot.py#L90-L104)
- [plot_service.py:107-121](file://apps/api/app/services/plot.py#L107-L121)
- [test_plots.py:189-226](file://apps/api/tests/test_plots.py#L189-L226)