...
- [apps/api/app/services/dashboard.py](file://apps/api/app/services/dashboard.py)
...
## 更新摘要
**变更内容**
- 新增农场仪表板API端点 GET /farms/{farm_id}/dashboard
- 添加仪表板服务层实现，解决N+1查询问题
- 新增仪表板相关的数据模型和响应结构
- 前端集成仪表板功能，提供统一的农场概览
...
## 简介
本文件为"农场管理"API的完整接口文档，覆盖农场的创建、查询、编辑、删除（通过成员退出实现），以及成员的邀请、角色变更、移除和退出等能力。**最新更新**新增了农场仪表板API，通过单个请求整合农场的近期操作、收获记录、成员信息和地块数据，有效解决了N+1查询性能问题。文档同时说明权限控制机制、请求/响应格式、错误码与异常处理，并给出基于测试用例的请求与响应示例路径。系统采用JWT鉴权与基于角色的访问控制（RBAC），并通过数据库事务与行级锁保证并发安全。
...
API --> Service["服务层<br/>farm.py, dashboard.py"]
...
- [apps/api/app/services/dashboard.py:16-70](file://apps/api/app/services/dashboard.py#L16-L70)
...
## 核心组件
- 路由与端点：提供农场与成员相关的所有HTTP接口，包括新增的仪表板端点
- 服务层：封装业务规则（如最小Owner数、角色权限、唯一性约束）和仪表板数据聚合逻辑
- 模型层：农场、成员、用户实体及角色枚举
...
### 农场仪表板
**新增** - 方法：GET
- 路径：/api/v1/farms/{farm_id}/dashboard
- 权限：必须是该农场成员（非成员视为不存在）
- 功能：获取农场仪表板数据，包括近期操作、近期收获、成员列表和地块信息
- 性能优化：通过单次请求整合多个数据源，避免N+1查询问题
- 成功响应：包含以下数据的聚合响应
  - recentOperations：最近的操作记录（最多5条）
  - recentHarvests：最近的收获记录（最多5条）
  - members：农场成员列表
  - plots：农场地块列表
- 失败响应：非成员返回未找到

**更新** 此接口通过单一数据库查询聚合多个数据源，显著提升性能：
- 一次性获取所有地块信息
- 批量查询农场成员及其用户信息
- 限制查询最近5条操作记录
- 限制查询最近5条收获记录
...
**章节来源**
- [apps/api/app/api/v1/endpoints/farms.py:195-226](file://apps/api/app/api/v1/endpoints/farms.py#L195-L226)
- [apps/api/app/services/dashboard.py:16-70](file://apps/api/app/services/dashboard.py#L16-L70)
- [apps/api/app/schemas/farm.py:186-204](file://apps/api/app/schemas/farm.py#L186-L204)
...
- **新增** 仪表板：GET /api/v1/farms/{farm_id}/dashboard，返回聚合的农场活动数据
...
- 异步I/O：使用异步会话提升吞吐
- **新增** 仪表板优化：通过单次请求聚合多个数据源，避免N+1查询问题
  - 批量查询农场所有地块
  - 联合查询成员与其用户信息
  - 限制操作和收获记录数量（默认5条）
  - 使用JOIN优化关联查询
...
## 结论
农场管理API提供了完整的CRUD与成员管理能力，结合JWT鉴权与基于角色的访问控制，确保数据安全与协作有序。通过行级锁与唯一性约束，系统在并发场景下保持数据一致性。**最新更新的仪表板功能**通过单次请求聚合多个数据源，显著提升了前端加载性能和用户体验。建议在生产环境严格配置JWT密钥与过期时间，并结合日志与监控快速定位问题。
...
### 仪表板数据模型
**新增** 仪表板相关的响应模型：
- DashboardOperationItem：包含操作记录和关联的地块信息
- DashboardHarvestItem：包含收获记录和关联的地块、行业信息
- FarmDashboardResponse：聚合所有仪表板数据的响应模型
