# 农事活动记录

<cite>
**本文引用的文件**
- [operation.py](file://apps/api/app/models/operation.py)
- [operation.py](file://apps/api/app/schemas/operation.py)
- [operation.py](file://apps/api/app/services/operation.py)
- [operations.py](file://apps/api/app/api/v1/endpoints/operations.py)
- [production.py](file://apps/api/app/models/production.py)
- [plot.py](file://apps/api/app/services/plot.py)
- [index.vue](file://apps/miniapp/src/pages/operations/index.vue)
- [form.vue](file://apps/miniapp/src/pages/operations/form.vue)
- [OperationForm.vue](file://apps/miniapp/src/components/OperationForm.vue)
- [operation.ts](file://apps/miniapp/src/services/operation.ts)
- [index.vue](file://apps/miniapp/src/pages/operation-types/index.vue)
- [plot.ts](file://apps/miniapp/src/services/plot.ts)
</cite>

## 更新摘要
**所做更改**
- 更新了 OperationForm 组件章节，反映集成的地块选择和增强的元数据显示
- 更新了作业创建与编辑章节，说明简化的表单流程和新的属性支持
- 更新了作业类型管理章节，描述从图标到文本界面的转变
- 增强了前端组件分析部分，包含新的 plot 属性和 plotSelectable 控制
- 更新了依赖关系分析，反映组件间的新交互模式

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与可扩展性](#性能与可扩展性)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录：标准化记录方法与数据分析方案](#附录标准化记录方法与数据分析方案)

## 简介
本模块为农场"农事活动记录"提供完整的前后端实现，覆盖作业类型定义、作业记录的创建/更新/删除、作业与生产（批次）的关联管理，以及地块维度的列表展示与表单录入。系统通过严格的模型约束与服务层校验保证数据完整性，并通过前端交互提升农事记录的易用性与一致性。**最新更新**：OperationForm 组件已进行重大重构，集成了地块选择功能，改进了表单结构和用户体验。

## 项目结构
围绕农事活动记录的核心代码分布在以下位置：
- 后端模型与Schema：定义作业类型与作业记录的数据结构与接口契约
- 后端服务层：封装业务规则、权限校验、时间有效性校验、与生产/地块/成员的关联校验
- 后端API端点：暴露REST接口，负责参数绑定、响应包装与分页
- 前端页面与组件：地块维度的作业列表、作业创建/编辑表单、作业类型选择页
- 前端服务层：统一HTTP调用与类型定义

```mermaid
graph TB
subgraph "前端"
OP_LIST["作业列表页<br/>pages/operations/index.vue"]
OP_FORM["作业表单页<br/>pages/operations/form.vue"]
OP_COMP["作业表单组件<br/>components/OperationForm.vue"]
OP_TYPES["作业类型页<br/>pages/operation-types/index.vue"]
OP_SVC["作业服务<br/>services/operation.ts"]
PLOT_SVC["地块服务<br/>services/plot.ts"]
end
subgraph "后端"
EP["API端点<br/>endpoints/operations.py"]
SVC["服务层<br/>services/operation.py"]
SCH["请求/响应Schema<br/>schemas/operation.py"]
MOD_OP["模型: 作业类型/作业记录<br/>models/operation.py"]
MOD_PROD["模型: 生产/工作法<br/>models/production.py"]
SVC_PLOT["服务层: 地块/成员校验<br/>services/plot.py"]
end
OP_LIST --> OP_SVC
OP_FORM --> OP_SVC
OP_TYPES --> OP_SVC
OP_COMP --> PLOT_SVC
OP_SVC --> EP
EP --> SVC
SVC --> MOD_OP
SVC --> MOD_PROD
SVC --> SVC_PLOT
```

**图表来源**
- [operations.py:63-168](file://apps/api/app/api/v1/endpoints/operations.py#L63-L168)
- [operation.py:139-307](file://apps/api/app/services/operation.py#L139-L307)
- [operation.py:18-80](file://apps/api/app/models/operation.py#L18-L80)
- [production.py:29-32](file://apps/api/app/models/production.py#L29-L32)
- [plot.py:107-121](file://apps/api/app/services/plot.py#L107-L121)

**章节来源**
- [operations.py:63-168](file://apps/api/app/api/v1/endpoints/operations.py#L63-L168)
- [operation.py:139-307](file://apps/api/app/services/operation.py#L139-L307)
- [operation.py:18-80](file://apps/api/app/models/operation.py#L18-L80)
- [production.py:29-32](file://apps/api/app/models/production.py#L29-L32)
- [plot.py:107-121](file://apps/api/app/services/plot.py#L107-L121)

## 核心组件
- 作业类型（OperationType）：用于标准化农事活动的分类，支持启用/禁用状态与排序
- 作业记录（FarmOperation）：记录一次农事操作，包含地块、可选的生产批次、作业类型、作业方式、作业时间、操作人、备注等
- 作业服务：负责创建、查询、更新、删除作业，并执行跨实体校验（地块权限、生产状态、时间有效性、操作人归属等）
- 前端作业列表：按地块维度展示作业记录，支持删除与编辑（受锁定策略限制）
- **更新的** 前端作业表单：集成地块选择功能，支持可配置的地块选择控制，增强的元数据显示
- 作业类型管理页：提供纯文本界面的作业类型选择入口

**章节来源**
- [operation.py:18-80](file://apps/api/app/models/operation.py#L18-L80)
- [operation.py:139-307](file://apps/api/app/services/operation.py#L139-L307)
- [index.vue:1-359](file://apps/miniapp/src/pages/operations/index.vue#L1-L359)
- [form.vue:1-263](file://apps/miniapp/src/pages/operations/form.vue#L1-L263)
- [OperationForm.vue:1-435](file://apps/miniapp/src/components/OperationForm.vue#L1-L435)
- [index.vue:1-78](file://apps/miniapp/src/pages/operation-types/index.vue#L1-L78)

## 架构总览
从用户操作到数据落库的端到端流程如下：

```mermaid
sequenceDiagram
participant U as "用户"
participant F as "前端页面/组件"
participant S as "前端服务(operation.ts)"
participant E as "API端点(operations.py)"
participant V as "服务层(operation.py)"
participant M as "数据库模型"
U->>F : 打开作业列表/表单
F->>S : 调用 getPlotOperations / createOperation / updateOperation
S->>E : HTTP 请求
E->>V : 解析参数并调用服务方法
V->>M : 读取/写入 作业类型/作业记录/生产/地块/成员
M-->>V : 返回结果或异常
V-->>E : 业务结果或错误
E-->>S : 响应数据
S-->>F : 渲染列表/提示成功
```

**图表来源**
- [operations.py:63-168](file://apps/api/app/api/v1/endpoints/operations.py#L63-L168)
- [operation.py:139-307](file://apps/api/app/services/operation.py#L139-L307)
- [operation.ts:55-90](file://apps/miniapp/src/services/operation.ts#L55-L90)

## 详细组件分析

### 数据模型设计
- 作业类型（OperationType）
  - 字段：唯一编码、名称、状态（ACTIVE/DISABLED）、排序号、创建/更新时间
  - 用途：标准化农事活动分类，便于前端展示与筛选
- 作业记录（FarmOperation）
  - 关键字段：地块ID、可选的生产批次ID、作业类型ID、作业方式（MANUAL/MECHANICAL）、作业时间、操作人ID、创建人ID、备注
  - 外键约束：与地块、生产、作业类型、用户建立强关联，确保引用完整性
- 生产（Production）与工作法（WorkMethod）
  - 生产包含状态（ACTIVE/ENDED）、开始/结束日期等；作业时间与生产开始日期的前后关系在服务层校验

```mermaid
erDiagram
OPERATION_TYPES {
bigint id PK
string code UK
string name
string status
int sort_order
datetime created_at
datetime updated_at
}
FARM_OPERATIONS {
bigint id PK
bigint plot_id FK
bigint production_id FK
bigint operation_type_id FK
string work_method
datetime operated_at
bigint operator_id FK
bigint created_by FK
text remark
datetime created_at
datetime updated_at
}
PRODUCTIONS {
bigint id PK
bigint plot_id FK
date started_on
date ended_on
string status
}
USERS {
bigint id PK
}
PLOTS {
bigint id PK
}
OPERATION_TYPES ||--o{ FARM_OPERATIONS : "被引用"
PLOTS ||--o{ FARM_OPERATIONS : "拥有"
PRODUCTIONS ||--o{ FARM_OPERATIONS : "可关联"
USERS ||--o{ FARM_OPERATIONS : "操作人/创建人"
```

**图表来源**
- [operation.py:18-80](file://apps/api/app/models/operation.py#L18-L80)
- [production.py:43-79](file://apps/api/app/models/production.py#L43-L79)

**章节来源**
- [operation.py:18-80](file://apps/api/app/models/operation.py#L18-L80)
- [production.py:43-79](file://apps/api/app/models/production.py#L43-L79)

### 类型与状态管理
- 作业类型状态：仅 ACTIVE 类型可用于新建/更新作业；DISABLED 类型不可选
- 生产状态：若作业关联的生产已结束（ENDED），则不允许再修改该作业
- 作业时间约束：
  - 不能晚于当前时间
  - 若关联生产，不能早于生产的开始日期

```mermaid
flowchart TD
Start(["提交作业"]) --> CheckType{"作业类型是否有效且启用?"}
CheckType --> |否| ErrType["返回错误: 类型无效或不可用"]
CheckType --> |是| CheckProd{"是否关联生产?"}
CheckProd --> |是| CheckProdStatus{"生产是否活跃?"}
CheckProdStatus --> |否| ErrProd["返回错误: 已结束的生产不可变更作业"]
CheckProdStatus --> |是| ValidateTime["校验作业时间范围"]
CheckProd --> |否| ValidateTime
ValidateTime --> TimeValid{"时间是否合法?"}
TimeValid --> |否| ErrTime["返回错误: 时间不合法"]
TimeValid --> |是| Save["保存作业记录"]
ErrType --> End(["结束"])
ErrProd --> End
ErrTime --> End
Save --> End
```

**图表来源**
- [operation.py:83-96](file://apps/api/app/services/operation.py#L83-L96)
- [operation.py:33-44](file://apps/api/app/services/operation.py#L33-L44)
- [operation.py:139-181](file://apps/api/app/services/operation.py#L139-L181)
- [operation.py:207-219](file://apps/api/app/services/operation.py#L207-L219)

**章节来源**
- [operation.py:83-96](file://apps/api/app/services/operation.py#L83-L96)
- [operation.py:33-44](file://apps/api/app/services/operation.py#L33-L44)
- [operation.py:139-181](file://apps/api/app/services/operation.py#L139-L181)
- [operation.py:207-219](file://apps/api/app/services/operation.py#L207-L219)

### 数据完整性保证
- 外键约束：作业记录必须关联有效的地块、作业类型、用户；生产可选但需满足所属地块一致
- 权限校验：
  - 操作人必须是地块所属农场的成员
  - 查询/编辑/删除均需验证当前用户对地块的访问权限
- 并发安全：
  - 更新/删除时采用行级锁（with_for_update）防止竞态条件
  - 对关联生产进行锁定检查，避免在并发下误改已结束生产
- 时间一致性：
  - 服务端强制将带时区的时间转换为无时区的UTC表示存储
  - 前端提交时需携带时区信息，否则校验失败

**章节来源**
- [operation.py:46-59](file://apps/api/app/services/operation.py#L46-L59)
- [operation.py:61-80](file://apps/api/app/services/operation.py#L61-80)
- [operation.py:184-204](file://apps/api/app/services/operation.py#L184-L204)
- [operation.py:222-289](file://apps/api/app/services/operation.py#L222-L289)
- [operation.py:292-307](file://apps/api/app/services/operation.py#L292-L307)

### 作业列表展示（前端）
- 功能要点：
  - 加载地块的作业列表、活跃生产、农场成员
  - 根据生产状态显示"已锁定"标记，禁止编辑/删除
  - 支持删除确认与错误提示
- 交互细节：
  - 未登录或令牌过期自动跳转登录
  - 空状态引导创建第一条记录
  - 图标映射不同作业类型

**章节来源**
- [index.vue:1-359](file://apps/miniapp/src/pages/operations/index.vue#L1-L359)

### 作业创建与编辑（前端）
**更新** 作业表单已进行重大重构，集成了地块选择功能并改进了用户体验：

- **集成的地块选择**：
  - 新增 `plot?: Plot | null` 属性，支持预填充地块信息
  - 新增 `plotSelectable?: boolean` 属性，控制地块选择的可编辑性
  - 在编辑模式下自动禁用地块选择，防止意外修改
- **增强的元数据显示**：
  - 显示地块类型（大田、水田、大棚等）和面积信息
  - 使用 `plotMeta()` 函数格式化地块元数据
  - 改进的视觉层次和信息密度
- **改进的表单结构**：
  - 统一的 `.form-item` 包装类，提供更好的间距和对齐
  - 标准化的字段标签和必填标识
  - 一致的输入控件样式和交互反馈
- **简化的页面流程**：
  - 移除了独立的地块选择字段，直接在表单中集成
  - 通过事件机制处理地块选择回调
  - 支持从其他页面传递选中地块参数

- **表单字段**：作业类型、关联生产、作业方式、作业日期、作业时间、操作人、备注
- **前置校验**：
  - 必填项校验（作业类型、操作人、时间等）
  - 作业时间不得早于关联生产的开始日期
  - 作业时间不得晚于当前时间
- **数据来源**：
  - 作业类型来自后端分页接口
  - 活跃生产列表用于限定可选范围
  - 农场成员列表用于选择操作人
- **提交逻辑**：
  - 新增：POST /plots/{plotId}/operations
  - 编辑：PATCH /operations/{operationId}
  - 成功后返回并关闭页面

**章节来源**
- [form.vue:1-263](file://apps/miniapp/src/pages/operations/form.vue#L1-L263)
- [OperationForm.vue:1-435](file://apps/miniapp/src/components/OperationForm.vue#L1-L435)
- [operation.ts:55-90](file://apps/miniapp/src/services/operation.ts#L55-L90)

### 作业类型管理（前端）
**更新** 作业类型页面已从图标显示改为纯文本界面：

- **作用**：在作业表单中提供作业类型选择入口
- **行为**：加载可用作业类型网格，点击后回传选中类型给父页面
- **界面改进**：
  - 移除了图标显示，改用简洁的文本布局
  - 保持了网格布局和点击交互
  - 统一的样式和响应式设计

**章节来源**
- [index.vue:1-78](file://apps/miniapp/src/pages/operation-types/index.vue#L1-L78)

### API 端点与响应
- 获取作业列表：GET /plots/{plot_id}/operations
- 创建作业：POST /plots/{plot_id}/operations
- 更新作业：PATCH /operations/{operation_id}
- 删除作业：DELETE /operations/{operation_id}
- 获取作业类型：GET /operation-types
- 统一响应格式：ApiResponse[data]，分页字段包含 items/page/pageSize/total

**章节来源**
- [operations.py:63-168](file://apps/api/app/api/v1/endpoints/operations.py#L63-L168)

## 依赖关系分析
**更新** 组件间的依赖关系已调整以支持新的地块选择功能：

- 服务层依赖：
  - 地块服务：校验用户对地块的访问权限
  - 生产服务：校验生产状态与所属地块一致性
  - 用户/农场成员：校验操作人归属
- 模型依赖：
  - 作业类型枚举与状态控制
  - 生产状态与工作法枚举
- **更新的** 前端依赖：
  - 作业服务封装HTTP调用
  - 地块服务提供地块详情和详细信息
  - 页面与组件通过事件与props传递上下文（地块、生产、成员、当前用户）
  - OperationForm 组件现在直接接收地块数据和选择控制

```mermaid
graph LR
SVC_OP["服务层: operation.py"] --> SVC_PLOT["服务层: plot.py"]
SVC_OP --> MOD_OP["模型: operation.py"]
SVC_OP --> MOD_PROD["模型: production.py"]
EP["端点: operations.py"] --> SVC_OP
FE_SVC["前端: operation.ts"] --> EP
FE_PAGE["前端页面/组件"] --> FE_SVC
OP_COMP["OperationForm组件"] --> PLOT_SVC["地块服务"]
OP_COMP --> FE_SVC
```

**图表来源**
- [operation.py:139-307](file://apps/api/app/services/operation.py#L139-L307)
- [plot.py:107-121](file://apps/api/app/services/plot.py#L107-L121)
- [operations.py:63-168](file://apps/api/app/api/v1/endpoints/operations.py#L63-L168)
- [operation.ts:55-90](file://apps/miniapp/src/services/operation.ts#L55-L90)

**章节来源**
- [operation.py:139-307](file://apps/api/app/services/operation.py#L139-L307)
- [plot.py:107-121](file://apps/api/app/services/plot.py#L107-L121)
- [operations.py:63-168](file://apps/api/app/api/v1/endpoints/operations.py#L63-L168)
- [operation.ts:55-90](file://apps/miniapp/src/services/operation.ts#L55-L90)

## 性能与可扩展性
- 分页查询：作业列表与作业类型均支持分页，减少单次传输数据量
- 索引优化：作业记录对地块、生产、作业类型、操作人建立索引，提升查询效率
- 并发控制：更新/删除使用行级锁，避免并发写冲突
- **更新的** 前端性能优化：
  - 集成的地块选择减少了页面跳转和数据重复加载
  - 改进的表单结构提升了渲染性能
  - 优化的事件处理减少了不必要的重新渲染
- 扩展建议：
  - 可按作业类型统计频次与成本，增加聚合视图
  - 可增加批量导入/导出能力，便于历史数据迁移
  - 可为作业记录增加附件（图片/视频）以增强现场记录

## 故障排查指南
- 常见错误与定位：
  - 作业类型无效或不可用：检查类型是否存在且状态为ACTIVE
  - 操作人不属于地块所属农场：确认操作人是否在农场成员列表中
  - 作业时间非法：检查是否晚于当前时间或与生产开始日期冲突
  - 已结束生产不可变更：确认关联生产状态是否为ACTIVE
  - 未授权（401）：检查登录态与令牌是否有效
  - **新增** 地块选择问题：检查 plotSelectable 属性设置和地块数据完整性
- 处理建议：
  - 前端捕获ApiRequestError并提示用户重试或重新登录
  - 后端通过统一的错误码与消息便于前端展示
  - 日志记录关键校验失败路径，便于问题复现
  - **新增** 对于地块选择相关错误，检查地块权限和用户访问控制

**章节来源**
- [operation.py:17-27](file://apps/api/app/services/operation.py#L17-L27)
- [operation.py:33-44](file://apps/api/app/services/operation.py#L33-L44)
- [operation.py:46-59](file://apps/api/app/services/operation.py#L46-L59)
- [operation.py:61-80](file://apps/api/app/services/operation.py#L61-L80)
- [index.vue:107-110](file://apps/miniapp/src/pages/operations/index.vue#L107-L110)
- [form.vue:97-100](file://apps/miniapp/src/pages/operations/form.vue#L97-L100)

## 结论
本模块通过清晰的模型设计与严格的服务层校验，实现了农事活动记录的标准化与安全性。**最新的重构**进一步提升了用户体验，OperationForm 组件的集成地块选择和改进的表单结构使得农事记录更加直观和高效。前端界面提供了直观的作业列表与表单，结合生产状态与权限控制，确保数据的一致性与可追溯性。后续可在统计分析、附件扩展与批量操作方面进一步增强用户体验与业务价值。

## 附录：标准化记录方法与数据分析方案
- 标准化记录方法
  - 作业类型：统一使用系统预置的类型（如施肥、灌溉、打药、消毒、换水等），避免自由文本导致统计困难
  - 作业方式：区分人工与机械，便于成本核算与效率分析
  - 作业时间：精确到分钟，并保证时区正确，避免跨时区数据混乱
  - 操作人：明确记录实际执行人与记录人，便于责任追溯
  - 关联生产：尽量关联到具体批次，以便按批次统计投入与产出
  - **更新的** 地块管理：通过集成的地块选择确保作业与地块的准确关联，支持地块类型和面积的标准化记录
- 数据分析方案
  - 作业频次统计：按作业类型、地块、时间段统计次数，识别高频作业与资源消耗点
  - 作业成本估算：结合作业方式与单位成本，计算地块/批次的投入成本
  - 作业与产出的关联：将作业与生产批次关联，分析投入对产量与质量的影响
  - 人员绩效：按操作人统计作业数量与类型分布，评估工作量与技能结构
  - 趋势分析：按月/季度汇总作业总量与类型占比，辅助生产计划与资源调度
  - **新增** 地块维度分析：基于集成的地块信息，分析不同类型地块的作业模式和投入产出比