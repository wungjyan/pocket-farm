...
- [apps/miniapp/src/services/home.ts](file://apps/miniapp/src/services/home.ts)
...
## 更新摘要
**变更内容**
- 新增首页服务层（home.ts），提供 getFarmDashboard 函数用于聚合农场数据获取
- 更新首页页面以使用新的聚合接口，显著提升前端性能
- 增强服务层架构，支持更高效的农场数据聚合与展示
...
## 简介
本技术文档聚焦 Pocket Farm 前端（uni-app）服务层，围绕 API 请求封装、错误处理、数据缓存策略展开，详细说明 HTTP 客户端配置、请求拦截器与响应处理器，并覆盖认证、农场、地块、生产、作业、收获、物种等业务模块的服务封装。特别新增了首页聚合服务层，通过 getFarmDashboard 函数实现农场数据的统一获取，显著提升了前端性能和用户体验。同时提供网络请求优化建议、重试机制设计思路以及离线数据处理方案，并给出扩展模式与第三方集成方案，帮助团队在现有基础上持续演进。

## 项目结构
前端服务层位于 apps/miniapp/src/services 目录下，采用"按业务域拆分"的组织方式：每个业务模块一个文件，统一通过 http.ts 中的 request 函数发起网络请求；认证相关逻辑集中在 auth.ts，用户会话由 user.ts 管理；farm-context.ts 提供当前农场的上下文状态与持久化。**新增的 home.ts 服务层专门处理首页聚合数据需求**。页面通过调用对应服务完成数据交互，如登录页调用 user.login，首页通过 home.getFarmDashboard 获取聚合数据。
...
- [apps/miniapp/src/services/home.ts:1-32](file://apps/miniapp/src/services/home.ts#L1-L32)
- [apps/miniapp/src/pages/home/index.vue:83-181](file://apps/miniapp/src/pages/home/index.vue#L83-L181)
...
- **首页聚合服务（home.ts）**
  - **新增功能**：提供 getFarmDashboard 函数，一次性获取农场的最近操作、最近收获、成员列表和地块信息。
  - 定义了 DashboardOperationItem、DashboardHarvestItem、FarmDashboardResponse 等类型接口。
  - 通过单一 API 调用 `/farms/${farmId}/dashboard` 获取聚合数据，显著提升前端性能。
...
- **新增首页聚合服务**：通过 home.ts 中的 getFarmDashboard 函数，将原本需要多次 API 调用的数据整合为单次请求，减少网络开销。
- 认证流程在 user.ts 中完成，登录成功后将令牌写入本地存储，后续请求自动带上 Authorization。
- 农场上下文在 farm-context.ts 中维护当前农场选择，结合 getMyFarms 进行同步与回退。

```mermaid
sequenceDiagram
participant Page as "首页(home/index.vue)"
participant HomeSvc as "home.ts"
participant Http as "http.ts"
participant Server as "后端API"
Page->>HomeSvc : getFarmDashboard(farmId)
HomeSvc->>Http : request({url : "/farms/{farmId}/dashboard"})
Http->>Server : 发送聚合数据请求
Server-->>Http : 返回{recentOperations, recentHarvests, members, plots}
Http-->>HomeSvc : 返回FarmDashboardResponse
HomeSvc-->>Page : 返回聚合数据
Page->>Page : 渲染首页动态和成员信息
```

**图表来源**
- [apps/miniapp/src/pages/home/index.vue:137-181](file://apps/miniapp/src/pages/home/index.vue#L137-L181)
- [apps/miniapp/src/services/home.ts:27-31](file://apps/miniapp/src/services/home.ts#L27-L31)
...
### 首页聚合服务（Home Service）
**新增功能**：首页聚合服务专门处理首页数据展示需求，通过单一 API 调用获取所有必要信息。

- **聚合数据接口**：getFarmDashboard(farmId) 函数调用 `/farms/${farmId}/dashboard` 端点
- **数据类型定义**：
  - `DashboardOperationItem`：包含操作记录、地块ID和名称
  - `DashboardHarvestItem`：包含收获记录、地块信息、行业类型
  - `FarmDashboardResponse`：聚合响应，包含最近操作、最近收获、成员列表、地块信息
- **性能优势**：相比分别调用多个 API，单次聚合请求减少了网络往返次数，提升加载速度
- **错误处理**：继承统一的 ApiRequestError 处理机制，支持 401 自动登出

```mermaid
flowchart TD
GetDashboard["getFarmDashboard(farmId)"] --> Request["request('/farms/{farmId}/dashboard')"]
Request --> Response["FarmDashboardResponse"]
Response --> Operations["recentOperations[]"]
Response --> Harvests["recentHarvests[]"]
...
- **home.ts 依赖 http.ts，并引用 operation、harvest、plot、farm 的类型定义**。
- 各业务服务均依赖 http.ts。
- farm-context.ts 依赖 farm.ts 与本地存储。
- 页面依赖对应服务，如 login.vue 依赖 user.ts 与 http.ts 的错误类型，**首页依赖 home.ts 获取聚合数据**。
...
User --> Auth
Home["home.ts"] --> Http
Home --> Operation["operation.ts"]
Home --> Harvest["harvest.ts"]
Home --> Plot["plot.ts"]
Home --> Farm["farm.ts"]
Farm["farm.ts"] --> Http
Plot["plot.ts"] --> Http
...
HomePage["home/index.vue"] --> Home
```

**图表来源**
- [apps/miniapp/src/services/home.ts:1-5](file://apps/miniapp/src/services/home.ts#L1-L5)
- [apps/miniapp/src/pages/home/index.vue:83-84](file://apps/miniapp/src/pages/home/index.vue#L83-L84)

## 性能与网络优化
当前实现已具备统一封装与错误处理，**新增的首页聚合服务进一步提升了性能**，建议在以下方面进一步优化：

- **聚合请求优化**
  - **已实现**：通过 home.ts 的 getFarmDashboard 函数，将多个数据源的请求合并为单次 API 调用
  - **效果**：首页加载从原来的多次网络请求减少为一次，显著降低首屏加载时间
- 请求去重
  - 对相同 URL+Method+Body 的请求在内存中进行合并，避免重复网络开销。
- 缓存策略
  - 读多写少接口（如 species、operation-types）增加短期内存缓存与失效策略（时间戳或版本号）。
  - 列表型数据可按资源维度做轻量级缓存，配合分页增量更新。
  - **首页聚合数据**：可对 dashboard 数据进行短期缓存，避免频繁刷新。
- 重试机制
...
- 页面侧处理
  ...
  - **首页页面**：对 dashboard 加载失败提供重试机制，支持用户手动刷新。
- 调试建议
  - 检查环境变量 VITE_API_BASE_URL 是否正确。
  - 确认本地是否已正确写入令牌。
  - 查看服务端返回的信封结构是否符合预期。
  - **首页调试**：检查 `/farms/{farmId}/dashboard` 端点返回的数据结构。

**章节来源**
- [apps/miniapp/src/services/http.ts:68-105](file://apps/miniapp/src/services/http.ts#L68-L105)
- [apps/miniapp/src/pages/home/index.vue:172-180](file://apps/miniapp/src/pages/home/index.vue#L172-L180)
- [apps/miniapp/src/pages/auth/login.vue:178-186](file://apps/miniapp/src/pages/auth/login.vue#L178-L186)

## 结论
Pocket Farm 前端服务层通过统一的请求封装、清晰的错误模型与分域服务划分，实现了高内聚、低耦合的网络层抽象。**新增的首页聚合服务层通过 getFarmDashboard 函数显著提升了前端性能**，将原本需要多次 API 调用的数据整合为单次请求，减少了网络开销并改善了用户体验。当前已具备完善的鉴权注入与信封解析能力，建议在缓存、重试、并发控制与离线优先等方面进一步增强，以提升用户体验与系统健壮性。
...
- **聚合服务模式**
  - **新实践**：参考 home.ts 的实现模式，为其他复杂页面创建聚合服务，减少前端数据组装复杂度。
  - **适用场景**：需要同时获取多个相关数据源的场景，如仪表盘、详情页等。

### 首页聚合服务最佳实践
- **数据聚合原则**
  - 将相关的业务数据聚合到单一 API 响应中，减少前端数据组装逻辑
  - 保持类型安全，通过 TypeScript 接口明确数据结构
- **性能优化**
  - 合理使用聚合接口，避免过度聚合导致响应体积过大
  - 考虑分页和懒加载，对大数据集进行分段处理
- **错误处理**
  - 继承统一的错误处理机制，确保异常情况的一致处理
  - 提供友好的错误提示和重试机制