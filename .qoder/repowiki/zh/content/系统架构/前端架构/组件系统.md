...
**更新** 新增了统一的导航指示器组件 PfRowChevron，用于替代各页面分散的箭头图标实现，并提供标准化的图像资源管理系统。
...
- 使用场景：所有可点击行的右侧导航指示，包括首页、农场页、我的页、地块详情页等。

**更新** 该组件是新增的统一导航指示器，替代了各页面分散的箭头图标实现，确保视觉一致性。
...
## 结论
Pocket Farm 前端组件系统以 Vue 3 的组合式 API 为核心，围绕表单组件构建了一套高内聚、低耦合的可复用体系。通过 props/事件/计算属性的清晰契约，结合服务层类型与工具函数，实现了稳定的数据流与一致的交互体验。与 uv-ui 的深度集成与设计令牌的统一管理，确保了视觉一致性与主题扩展能力。**新增的 PfRowChevron 组件进一步统一了导航指示器的视觉表现，提升了用户体验的一致性。** 遵循本文的开发规范与最佳实践，可在保证可维护性的同时快速扩展新的业务表单与页面。
...
- **在页面中使用 PfRowChevron：**
  ...
  - 适用于所有可点击行的右侧导航指示，如首页、农场页、我的页、地块详情页等。
  - 组件会自动处理图片加载和样式，确保 30rpx × 30rpx 的统一尺寸。
  - 示例路径参考：[home/index.vue:72](file://apps/miniapp/src/pages/home/index.vue#L72)、[mine/index.vue:14](file://apps/miniapp/src/pages/mine/index.vue#L14)
- 与 uv-ui 集成与主题定制：
...
- [mine/index.vue:14](file://apps/miniapp/src/pages/mine/index.vue#L14)
