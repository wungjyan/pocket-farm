...
- [apps/miniapp/src/pages/harvests/form.vue](file://apps/miniapp/src/pages/harvests/form.vue)
...
## 更新摘要
**变更内容**
- 新增 farm-plots 页面用于农场地块浏览，提供筛选和统计功能
- plots 页面现在专注于选择模式，支持事件通道返回选中的地块
- 首页导航到 farm-plots 页面并携带 ALL 过滤器参数
- 操作和生产表单直接导航到 plots 选择页面
- 路由配置中的导航标题已更新为"选择地块"
- 更新了地块管理相关的导航流程和交互逻辑
...
G --> H["farm-plots页面<br/>地块列表浏览"]
G --> I["plots页面<br/>地块选择模式"]
```
...
- 地块管理页面：farm-plots 页面提供地块浏览、筛选和管理功能。
- 地块选择页面：plots 页面专注于地块选择，支持事件通道返回选中结果。
...
participant FP as "farm-plots页面"
participant PS as "plots选择页面"
M->>A : 创建并挂载应用
A->>A : onLaunch()
A->>A : hasAuthToken()?
alt 已登录
...
- **新增页面**：farm-plots 页面用于地块列表浏览，plots 页面用于地块选择模式。
...
Route --> NewPages{"新地块管理页面?"}
NewPages -- 是 --> FarmPlots["farm-plots: 地块列表"]
NewPages -- 否 --> Other["其他页面"]
FarmPlots --> Plots["plots: 地块选择"]
```
...
图表来源
- [apps/miniapp/src/pages/home/index.vue:143-146](file://apps/miniapp/src/pages/home/index.vue#L143-L146)
- [apps/miniapp/src/pages/farm/index.vue:157-160](file://apps/miniapp/src/pages/farm/index.vue#L157-L160)
...
- 事件通道：使用 getOpenerEventChannel 实现页面间双向通信，如地块选择页面返回选中结果。
- 示例路径：
  - 首页 -> 开始种养：携带 farmId
  - 首页 -> 记录农事：携带 farmId
  - 首页 -> 记录收获：携带 farmId
  - 首页 -> 地块列表：携带 farmId 和 filter=ALL
  - 农场页 -> 地块列表：携带 farmId 和 filter 参数
  - 操作/生产表单 -> 地块选择：携带 farmId 和 selectedPlotId
  - 农场页 -> 生产详情：携带 productionId
- 注意事项：
  - 对敏感或复杂对象不建议仅通过 URL 传递，应结合本地存储或全局状态。
  - 跨 Tab 页面无法使用 navigateTo，需使用 switchTab。
  - 事件通道适用于需要返回数据的场景，如地块选择。

章节来源
- [apps/miniapp/src/pages/home/index.vue:193-209](file://apps/miniapp/src/pages/home/index.vue#L193-L209)
- [apps/miniapp/src/pages/farm/index.vue:172-174](file://apps/miniapp/src/pages/farm/index.vue#L172-L174)
- [apps/miniapp/src/pages/operations/form.vue:178-195](file://apps/miniapp/src/pages/operations/form.vue#L178-L195)
- [apps/miniapp/src/pages/productions/form.vue:118-134](file://apps/miniapp/src/pages/productions/form.vue#L118-L134)
- [apps/miniapp/src/pages/plots/index.vue:112-127](file://apps/miniapp/src/pages/plots/index.vue#L112-L127)

### 路由守卫：数据预取与权限验证
- 数据预取：
  - 首页：onShow 中并发获取当前用户信息、刷新农场上下文，并加载最近动态。
  - 农场页：onShow 中刷新农场上下文并加载概览数据。
  - 地块管理页：onShow 中加载地块列表和活跃生产信息。
- 权限验证：
...
```mermaid
sequenceDiagram
participant Page as "页面(首页/农场/地块)"
...
图表来源
- [apps/miniapp/src/pages/home/index.vue:239-247](file://apps/miniapp/src/pages/home/index.vue#L239-L247)
- [apps/miniapp/src/pages/farm/index.vue:188-198](file://apps/miniapp/src/pages/farm/index.vue#L188-L198)
- [apps/miniapp/src/pages/farm-plots/index.vue:203-210](file://apps/miniapp/src/pages/farm-plots/index.vue#L203-L210)
- [apps/miniapp/src/pages/auth/login.vue:158-187](file://apps/miniapp/src/pages/auth/login.vue#L158-L187)
...
- **地块管理优化**：
  - farm-plots 页面支持按种类和空闲状态筛选，提升查找效率。
  - plots 选择页面使用事件通道返回选中结果，避免复杂的参数传递。

章节来源
- [apps/miniapp/src/components/PfPageHeader.vue:37-43](file://apps/miniapp/src/components/PfPageHeader.vue#L37-L43)
- [apps/miniapp/src/composables/useUnsavedChangesGuard.ts:1-27](file://apps/miniapp/src/composables/useUnsavedChangesGuard.ts#L1-L27)
- [apps/miniapp/src/pages/farm-plots/index.vue:115-128](file://apps/miniapp/src/pages/farm-plots/index.vue#L115-L128)
- [apps/miniapp/src/pages/plots/index.vue:112-115](file://apps/miniapp/src/pages/plots/index.vue#L112-L115)
...
- **功能特性**：
  ...
  - 支持按物种类型和空闲状态筛选
  - 显示地块面积、类型和当前种植物种
  - 提供创建地块功能（仅限农场所有者和管理员）
- **导航集成**：
  - 首页和农场页导航到此页面，携带 farmId 和 filter 参数
  - 点击地块条目导航到地块详情页
  - 支持创建新地块导航
...
- **功能特性**：
  ...
  - 提供创建地块快捷入口
- **导航集成**：
  - 操作表单和生产表单导航到此页面进行地块选择
  - 通过事件通道将选中的地块返回给调用页面
  - 支持传入 selectedPlotId 进行预选
...
- **新增依赖关系**：
  - farm-plots 页面依赖 plot 和 production 服务获取地块和活跃生产信息
  - plots 页面依赖 plot 服务获取地块列表
  - 操作和生产表单依赖 plots 页面进行地块选择
...
Home --> |navigateTo| FarmPlots["farm-plots/index.vue"]
Farm --> |navigateTo| FarmPlots
FarmPlots --> |navigateTo| Plots["plots/index.vue"]
Operations["operations/form.vue"] --> |navigateTo| Plots
Productions["productions/form.vue"] --> |navigateTo| Plots
Plots --> |事件通道| Operations
Plots --> |事件通道| Productions
```
...
- **地块管理优化**：
  - farm-plots 页面使用计算属性进行实时筛选，提升用户体验。
  - plots 选择页面简化了地块选择流程，减少不必要的页面跳转。
...
- 常见问题
  ...
  - 地块选择失败：检查事件通道是否正确设置和处理。
  - 筛选功能异常：确认 farm-plots 页面的筛选逻辑和数据绑定。
- 定位方法
...
## 结论
本项目采用 UniApp 原生路由体系，通过 pages.json 集中管理页面与 TabBar，结合 App.vue 的生命周期钩子实现启动期鉴权与初始跳转；业务页面在 onShow 中完成数据预取与权限校验，并通过 uni 导航 API 完成页面间跳转与传参。新增的地块管理页面提供了更完善的地块浏览和选择功能，提升了用户体验和操作效率。整体结构清晰、职责明确，便于扩展与维护。
...
- **新增地块管理场景**：
  - 首页导航到地块列表：携带 farmId 和 filter=ALL 参数。
  - 地块选择：使用事件通道返回选中的地块对象。
  - 筛选地块：支持按物种类型和空闲状态进行筛选。
  - 创建地块：从地块列表直接进入创建页面。
