...
- 领域模型
  - 用户、农场、地块、生产、作业、收获等实体，包含主键、唯一键、外键与部分显式索引。
- 服务层
...
- 外键索引
  - 多处外键列未显式声明 index=True，但多数作为查询过滤条件或 JOIN 列，建议为高频外键列添加索引以提升 JOIN 与 WHERE 性能。
- 复合索引
...
## 结论
Pocket Farm 在后端服务层已较好地避免了 N+1 问题，并通过 JOIN、分页与锁行提升了查询效率与并发安全性。下一步应重点完善索引体系（尤其是外键与复合索引）、持续监控慢查询与连接池状态，并在大规模数据场景下引入更稳健的分页与归档策略。配合合理的缓存与事务边界控制，可进一步提升系统吞吐与稳定性。
